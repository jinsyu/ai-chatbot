...
const TEXT_TO_SQL_API_URL =
  process.env.TEXT_TO_SQL_API_URL || 'http://localhost:6001';
...
export const textToSql = ({ session, dataStream }: TextToSqlProps) =>
  tool({
    ...
    inputSchema: z.object({
      query: z
        .string()
        .describe('Natural language query to execute on the database'),
      maxRows: z
        .number()
        .optional()
        .default(1000)
    ...
    execute: async ({ query, maxRows }) => {
      ...
      try {
        ...
        console.log('[Text-to-SQL] API Response:', response);

        if (!response.ok) {
          const errorBody = await response.text();
          console.error('[Text-to-SQL] API Error Response:', errorBody);
          throw new Error(`API error: ${response.statusText} - ${errorBody}`);
        }

        const result: TextToSqlResponse = await response.json();
        console.log(
          '[Text-to-SQL] API Response:',
        ...
        if (result.success && !result.error) {
          ...
          if (result.results && result.results.length > 0) {
            ...
            console.log(
              '[Text-to-SQL] Creating Sheet artifact with',
              queryData.length,
              'rows',
            );
            ...
            console.log(
              '[Text-to-SQL] CSV data (first 200 chars):',
          ...
          const enhancedMessage =
            result.response ||
            `Query executed successfully. Retrieved ${result.row_count} rows.`;
