...
class InventoryDataImporter:
    def __init__(self, excel_path: str, db_path: str = "db.sqlite"):
        ...
        self.engine = None
        self.table_name = 'sap_zmmr0016_inventory'
        self.df = None
    ...
    def analyze_excel(self):
        ...
        try:
            ...
            # 마지막 행이 합계 행인지 확인 (보통 자재명이 '합계' 또는 비어있음)
        ...
        try:
            ...
            with self.engine.connect() as conn:
                ...
                for col_info in schema_result:
                    ...
                    # 재고 관련 컬럼이면 합계 계산
                    if '재고' in col_name or '수량' in col_name:
                        try:
                    ...
                    # 플랜트나 자재 컬럼이면 고유값 수 계산
                    elif '플랜트' in col_name or '자재' in col_name:
                        try:
    ...
    def run(self):
        ...
        try:
            logger.info("=" * 60)
            logger.info("ZMMR0016 재고 데이터 Import (SQLite)")
            logger.info("=" * 60)
...
if __name__ == "__main__":
    excel_file = "/Users/jinsyu/Downloads/ZMMR0016_제상품재고현황.XLSX"
    db_file = "db.sqlite"  # SQLite 데이터베이스 파일
    
    if not os.path.exists(excel_file):
        logger.error(f"파일을 찾을 수 없습니다: {excel_file}")
    else:
        importer = InventoryDataImporter(excel_file, db_file)
        importer.run()