...
class SalesOrderImporter:
    def __init__(self, excel_path: str, db_path: str = "db.sqlite"):
        ...
        self.engine = None
        self.table_name = 'sap_zsdr0062_sales_orders'
        self.df = None
    ...
    def analyze_excel(self):
        ...
        try:
            ...
            df_sample = pd.read_excel(self.excel_path, sheet_name=0, nrows=10)
            logger.info(f"\n📋 샘플 데이터 (처음 10행):")
            ...
            logger.info(f"\n✅ 데이터 로드 완료")
            logger.info(f"  - 원본 행 수: {len(self.df)}")
            logger.info(f"  - 열 수: {len(self.df.columns)}")
            
            # 판매오더 컬럼 찾기 (판매오더, 판매 오더, 오더, Sales Order 등)
            order_column = None
            for col in self.df.columns:
                if '판매' in str(col) and '오더' in str(col):
                    order_column = col
                    break
                elif '오더' in str(col) or 'order' in str(col).lower():
                    order_column = col
                    break
            
            if order_column:
                logger.info(f"\n🔍 판매오더 컬럼 발견: '{order_column}'")
                ...
                # 판매오더가 비어있거나 0인 행 제외
                self.df = self.df[
                    (self.df[order_column].notna()) & 
                    (self.df[order_column] != '') &
                    (self.df[order_column] != 0) &
                    (self.df[order_column] != '0')
                ]
                ...
                logger.info(f"  ⚠️ 판매오더 없는 행 {excluded_count}개 제외됨")
                logger.info(f"  - 제외 전: {before_count}개")
                logger.info(f"  - 제외 후: {after_count}개")
            else:
                logger.warning("  ⚠️ 판매오더 컬럼을 찾을 수 없습니다. 모든 데이터를 포함합니다.")
            
            # 마지막 행이 합계 행인지 확인
            last_row = self.df.iloc[-1] if len(self.df) > 0 else None
            if last_row is not None:
                # 첫 번째 컬럼이 비어있거나 '합계'가 포함되어 있는지 확인
            ...
            logger.info(f"\n📋 최종 데이터:")
            logger.info(f"  - 최종 행 수: {len(self.df)}")
            ...
            if len(self.df) > 0:
                ...
                with self.engine.connect() as conn:
                    ...
                    try:
                        # 컬럼명에 따라 인덱스 생성
                        for col in self.df.columns:
                            if '판매' in col and '오더' in col:
                                conn.execute(text(f"CREATE INDEX idx_{self.table_name}_sales_order ON {self.table_name} (`{col}`)"))
                                logger.info(f"  인덱스 생성: 판매오더")
                            elif '고객' in col or '판매처' in col:
                                conn.execute(text(f"CREATE INDEX idx_{self.table_name}_customer ON {self.table_name} (`{col}`)"))
                                logger.info(f"  인덱스 생성: 고객/판매처")
                            elif '자재' in col and ('번호' in col or '코드' in col):
                                conn.execute(text(f"CREATE INDEX idx_{self.table_name}_material ON {self.table_name} (`{col}`)"))
                                logger.info(f"  인덱스 생성: 자재")
                            elif '납품일' in col or '출하일' in col:
                                conn.execute(text(f"CREATE INDEX idx_{self.table_name}_delivery_date ON {self.table_name} (`{col}`)"))
                                logger.info(f"  인덱스 생성: 납품/출하일")
                        conn.commit()
                    except Exception as e:
                ...
                logger.error("데이터가 없어 테이블을 생성할 수 없습니다.")
                return False
        ...
        try:
            ...
            with self.engine.connect() as conn:
                ...
                for col_info in schema_result:
                    ...
                    # 판매오더 컬럼이면 고유값 수 계산
                    if '판매' in col_name and '오더' in col_name:
                        try:
                    ...
                    # 금액 관련 컬럼이면 합계 계산
                    elif '금액' in col_name or '가격' in col_name:
                        try:
                            sum_query = f"SELECT SUM(`{col_name}`) FROM {self.table_name} WHERE `{col_name}` IS NOT NULL"
                            sum_result = conn.execute(text(sum_query)).scalar()
                            if sum_result:
                                logger.info(f"  {col_name} 합계: {sum_result:,.0f}")
                        except:
                            pass
                    
                    # 고객/판매처 컬럼이면 고유값 수 계산
                    elif '고객' in col_name or '판매처' in col_name:
                        try:
                            distinct_query = f"SELECT COUNT(DISTINCT `{col_name}`) FROM {self.table_name}"
                            distinct_result = conn.execute(text(distinct_query)).scalar()
                            logger.info(f"  {col_name} 고유값: {distinct_result:,}개")
    ...
    def run(self):
        ...
        try:
            logger.info("=" * 60)
            logger.info("ZSDR0062 판매오더별현황 데이터 Import (SQLite)")
            logger.info("=" * 60)
            ...
            # 데이터가 있는지 확인
            if len(self.df) == 0:
                logger.warning("판매오더가 있는 데이터가 없습니다. Import를 중단합니다.")
                return False
...
if __name__ == "__main__":
    excel_file = "/Users/jinsyu/Downloads/ZSDR0062_판매오더별현황.XLSX"
    db_file = "db.sqlite"  # SQLite 데이터베이스 파일
    
    if not os.path.exists(excel_file):
        logger.error(f"파일을 찾을 수 없습니다: {excel_file}")
    else:
        importer = SalesOrderImporter(excel_file, db_file)
        importer.run()